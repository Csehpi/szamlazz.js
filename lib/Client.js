...
export class Client {
  ...
  async _sendRequest (fileFieldName, data, isBinaryDownload) {
    ...
    formData.append(fileFieldName, data)
    ...
    const httpResponse = await this.#axiosInstance.post('https://www.szamlazz.hu/szamla/', formData.getBoundary('action-xmlagentxmlfile'), axiosOptions)
